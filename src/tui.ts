...
function element(tag: string, props: Record<string, unknown> = {}, children: unknown[] = []): Element {
...
const box = (props: Record<string, unknown>, children: unknown[] = []): Element =>
...
function header(theme: Theme, version: string): Element {
...
function toggleRow(theme: Theme, expanded: boolean, count: number, onToggle: () => void): Element {
...
function emptyState(theme: Theme, routes: Route[], current: Route | undefined, expanded: boolean): Element[] {
  if (routes.length === 0) {
...
function readRoutes(ctx: TuiContext, location: TuiLocation): Route[] {
  ...
  return models
    .filter(
      (model) =>
        model.providerID === ROUTER_PROVIDER && typeof model.body?.model === "string",
    )
 ...
 */
function readSelection(ctx: TuiContext, location: TuiLocation): SelectedModel | undefined {
