 ...
 */
function currentSelection(ctx: TuiContext): SelectedModel | undefined {
  const ui = ctx.ui as {
    model?: { current?: () => SelectedModel | undefined };
  };
  trace(`probe ui.keys=${Object.keys(ctx.ui).join(",")}`);
  trace(
    `probe ui.model typeof=${typeof ui.model} keys=${Object.keys(ui.model ?? {}).join(",")}`,
  );
  try {
...
async function report(ctx: TuiContext): Promise<void> {
  const location = ctx.location ?? ctx.data.location.default();
  const selected = currentSelection(ctx);
  trace(
    `report location=${JSON.stringify(location)} selected=${JSON.stringify(selected)}`,
  );
  ...
  const target = models.find(
    (model) =>
      model.providerID === OPENCODE_PROVIDER && model.id === targetID,
  );
